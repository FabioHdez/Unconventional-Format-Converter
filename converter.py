...
def main(id:str,input:list,format:str):
    ...
    """
    try:
        os.mkdir(f"{os.path.dirname(__file__)}/output/{id}")
    except:
        print("Folder already exists.")
    for i in input:
        name = i.split(".")[0]
        # ffmpeg -y -i input.mov -v quiet af23s2/output.mp4
        convertCommand = ['ffmpeg','-y','-i', f"./{i}",'-v','quiet', f"output/{id}/{name}{format}"]
        subprocess.run(convertCommand)

if __name__ == "__main__":
    main('c96c6d5b',['input.webm','input2.webm','input3.webm','input4.webm'],'.mp4')